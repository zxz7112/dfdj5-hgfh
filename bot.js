...
client.on('ready', () => {
  client.user.setGame(` By zxz7111 .`);
  console.log('---------------');
...
if (message.content === '*help') {
         let embed = new Discord.RichEmbed()
.setThumbnail(message.author.avatarURL)    
      .addField("** مبرمج البوت :wrench: **","**@!      zxz7111  **")
.setColor('RANDOM')
  message.author.sendEmbed(embed);
    }
});
          ...
          .then(co => {
            text = co.first().content
              ...
              message.channel.send(`تم حفظ اقتراحك الرجاء انتضار الرد من قبل الاداره`)
                client.channels.get("530229926834733077").send(`${message.author.username}'s sug => ${text}`)
...
client.on('message',  message => {
  if(bannedwords.some(word => message.content.includes(word))) {
    message.delete()
    message.reply("  ألفاظك يا سيد يا محترم :slight_smile:    ").then(msg => {msg.delete(5000)});;
  };
});
...
client.on('ready', () => {
   console.log(`----------------`);
      console.log(`Desert Bot- Script By : zxz7111`);
        console.log(`----------------`);
      console.log(`ON ${client.guilds.size} Servers '     Script By : zxz711 ' `);
    console.log(`----------------`);
  console.log(`Logged in as ${client.user.tag}!`);
client.user.setGame(`  *help لمعرفة الاومر اكتب`,"http://twitch.tv/zsz711")
...
client.channels.find('name',"big-bot").send(embed)
});
...
client.on('message', function(message) {
    if(message.content.startsWith("*report")) {
        ...
        if(!messageReason) return message.reply("**# اكتب سبب البلاغ!**");
    let mUser = message.mentions.users.first();
    if(!mUser) return message.channel.send("لايمكن ايجاد الشخص.");
...
message.channel.send("__ هل انت متاكد لارسال هذا البلاغ لصاحب السيرفر ??__").then(msg => {
    msg.react("✅")
    msg.react("❌")
.then(() => msg.react('❌'))
.then(() =>msg.react('✅'))
...
client.on("ready", () => {
    var guild;
    while (!guild)
        guild = client.guilds.get("530220677286985769");
...
client.on("guildMemberAdd", (member) => {
    let channel = member.guild.channels.get("540307317858631701");
    if (!channel) {
